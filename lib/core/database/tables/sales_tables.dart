...
class SalesInvoices extends Table {
  ...
  RealColumn get totalDiscount => real().withDefault(Constant(0.0))();
  RealColumn get totalAmount => real()(); // final billed amount
  RealColumn get amountPaid => real().withDefault(Constant(0.0))(); // Tracks partial clearings
  RealColumn get creditBalanceAdded => real().withDefault(Constant(0.0))(); // Outstanding debt logged from this bill
  TextColumn get customerNotes => text().nullable()(); // Custom follow-up payment text reminders
  TextColumn get paymentMode =>
      textEnum<PaymentMode>().withDefault(Constant('cash'))();
}
...
class SalesInvoiceItems extends Table {
  ...
  IntColumn get totalTabletsSold => integer().withDefault(Constant(1))(); // Tracks individual pill counts cut from strips
  RealColumn get mrpPerTablet => real().withDefault(Constant(0.0))();
  RealColumn get gstPercentage => real()();
  RealColumn get discountPercent => real().withDefault(Constant(0.0))();
